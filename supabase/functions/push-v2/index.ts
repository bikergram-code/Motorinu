...
async function getAccessToken(): Promise<string> {
  ...
  let saJson = Deno.env.get('FIREBASE_SERVICE_ACCOUNT_JSON');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[Push-v2] len=' + raw.length);
    if (raw.length < 5) {
      return new Response(JSON.stringify({ error: 'empty body', len: raw.length }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (table === 'notifications') {
      ...
      else if (mt === 'image' || rec.image_url) prev = '\ud83d\uddbc\ufe0f Bild';
