...
class StoryCommentsSheet extends ConsumerStatefulWidget {
  const StoryCommentsSheet({super.key, required this.storyId, this.storyUserId});

  final int storyId;
  final String? storyUserId;

  static Future<void> show(BuildContext context, {required int storyId, String? storyUserId}) {
    return showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => StoryCommentsSheet(storyId: storyId, storyUserId: storyUserId),
    );
  }
...
class _StoryCommentsSheetState extends ConsumerState<StoryCommentsSheet> {
  ...
  Future<void> _loadComments() async {
    ...
    try {
      ...
      final comments = await repo.getComments(widget.storyId);
      if (!mounted) return;
      setState(() {
        _comments = comments;
        _isLoading = false;
  ...
  Future<void> _sendComment() async {
    ...
    try {
      final repo = ref.read(storyRepositoryProvider);
      final comment = await repo.addComment(widget.storyId, text);
      _controller.clear();
      if (!mounted) return;
      setState(() {
      ...
      if (widget.storyUserId != null && widget.storyUserId!.isNotEmpty) {
        ...
        debugPrint('[StoryCommentsSheet] communityProvider = $community, name = ${community?.name}');
        ref.read(notificationRepositoryProvider).createNotification(
          targetUserId: widget.storyUserId!,
          type: 'comment',
          title: '$myName hat kommentiert',
          body: text.length > 80 ? '${text.substring(0, 80)}...' : text,
          data: {'story_id': widget.storyId, 'actor_id': myId},
  ...
  Future<void> _deleteComment(StoryComment comment) async {
    try {
      final repo = ref.read(storyRepositoryProvider);
      await repo.deleteComment(comment.id, widget.storyId);
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildComment(StoryComment comment, Color accentColor, Color textOnCard) {
    return Padding(
