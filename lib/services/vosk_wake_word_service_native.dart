...
class VoskWakeWordService {
  ...
  /// At distance, Vosk often hears "hi" as "die", "wie", "nie", "sie", "wir" etc.
  static const _wakePrefixes = [
    'hi', 'hey', 'hallo', 'hej', 'he', 'ha',
    // Misheard at distance:
    'die', 'wie', 'nie', 'sie', 'wir', 'mir', 'hier',
    'bei', 'dei', 'ei', 'ai',
  ];
  ...
  static const _motoVariants = [
    ...
    'mordo', 'morto', 'mojo', 'mono', 'mozo', 'mord', 'morgen',
    'foto', 'photo', // "moto" misheard as "foto"
  ];

  /// Standalone triggers — detected WITHOUT prefix.
  /// "moto" alone (without "hi") should also trigger at distance.
  static const _standaloneTriggers = [
    'moto', 'motto', 'motor', 'hi moto', 'hey moto',
  ];
  ...
  Future<void> startListening() async {
    ...
    try {
      ...
      // Enable 5 alternatives for better wake word matching at distance
      await _recognizer!.setMaxAlternatives(5);
      ...
      debugPrint('[Vosk] Listening started (wake word mode, 5 alternatives)');
    } catch (e) {
  ...
  /// Check if text contains a wake word pattern.
  /// Matches: [prefix] + [moto variant], OR standalone "moto"/"motto"/"motor".
  bool _containsWakeWord(String text) {
    final words = text.split(RegExp(r'\s+'));

    // 1. Check standalone triggers — "moto" alone without prefix
    for (final word in words) {
      if (_standaloneTriggers.contains(word)) {
        return true;
      }
    }

    // 2. Check prefix + moto pattern: "hi motor", "die motto", etc.
    ...
    // 3. Fuzzy: any two-syllable word starting with "mo" after any short word
    for (int i = 0; i < words.length - 1; i++) {
      if (words[i].length <= 4) { // short prefix word
        final next = words[i + 1];
        if (next.startsWith('mot') && next.length >= 4) {
          return true; // "X motto", "X motor", "X moto" etc.
        }
      }
    }
  ...
  int _findWakeWordEnd(String text) {
    ...
    for (int i = 0; i < words.length; i++) {
      ...
      // Standalone trigger: "moto tankstelle" → command = "tankstelle"
      if (_standaloneTriggers.contains(words[i])) {
        return charPos + words[i].length;
      }

      // Prefix + moto: "hi moto tankstelle" → command = "tankstelle"
      if (i < words.length - 1 && _wakePrefixes.contains(words[i])) {
        final next = words[i + 1];
        if (_motoVariants.contains(next) || (next.startsWith('mo') && next.length >= 3)) {
