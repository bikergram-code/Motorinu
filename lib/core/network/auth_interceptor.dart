...
/// Rotation-safe Auth Interceptor:
/// - adds Authorization header
/// - on 401: refresh (single-flight) -> save NEW access+refresh -> retry request
class AuthInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (newPair == null) {
      await _tokenStore.clear(reason: 'refresh_failed');
      return handler.next(err);
    }
  ...
  Future<TokenPair?> _refreshTokens() async {
    ...
    () async {
      try {
        ...
        await _tokenStore.write(rotated, reason: 'interceptor_refresh');
