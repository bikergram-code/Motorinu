...
class BlitzerSettings {
  ...
  // Navigation
  final bool autoRecalculate; // Automatisch neu berechnen bei Off-Route
  final int offRouteThreshold; // Meter bis Off-Route erkannt (30-100)
  final bool showBlitzerOnRoute; // Blitzer auf der Route anzeigen

  // TTS (Text-to-Speech)
  final bool ttsEnabled; // Sprachansagen für Abbiegehinweise
  final String ttsVoice; // Stimmprofil: 'standard', 'pirat', 'biker', 'robot', 'opa', 'drill'
  ...
  const BlitzerSettings({
    ...
    this.showBlitzerOnRoute = true,
    this.ttsEnabled = true,
    this.ttsVoice = 'standard',
  ...
  BlitzerSettings copyWith({
    ...
    return BlitzerSettings(
      ...
      autoRecalculate: autoRecalculate ?? this.autoRecalculate,
      offRouteThreshold: offRouteThreshold ?? this.offRouteThreshold,
      showBlitzerOnRoute: showBlitzerOnRoute ?? this.showBlitzerOnRoute,
...
class BlitzerSettingsNotifier extends AsyncNotifier<BlitzerSettings> {
  ...
  Future<BlitzerSettings> build() async {
    ...
    return BlitzerSettings(
      ...
      autoRecalculate: prefs.getBool('${_prefix}auto_recalculate') ?? true,
      offRouteThreshold: prefs.getInt('${_prefix}off_route_threshold') ?? 50,
      showBlitzerOnRoute: prefs.getBool('${_prefix}show_on_route') ?? true,
      ttsEnabled: prefs.getBool('${_prefix}tts_enabled') ?? true,
      ttsVoice: prefs.getString('${_prefix}tts_voice') ?? 'standard',
  ...
  Future<void> _persist(BlitzerSettings s) async {
    ...
    await Future.wait([
      ...
      prefs.setBool('${_prefix}auto_recalculate', s.autoRecalculate),
      prefs.setInt('${_prefix}off_route_threshold', s.offRouteThreshold),
      prefs.setBool('${_prefix}show_on_route', s.showBlitzerOnRoute),
      prefs.setBool('${_prefix}tts_enabled', s.ttsEnabled),
      prefs.setString('${_prefix}tts_voice', s.ttsVoice),
