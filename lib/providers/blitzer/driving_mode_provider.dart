...
import 'blitzer_settings_provider.dart';
...
class DrivingModeState {
  ...
  /// Blitzer reports on the current route.
...
class DrivingModeNotifier extends Notifier<DrivingModeState> {
  ...
  /// Calculate blitzer reports along the route.
  void setBlitzerOnRoute(List<BlitzerReport> reports, List<LatLng> polyline) {
    final onRoute = BlitzerAlertService.findBlitzerOnRoute(
      reports: reports,
      polyline: polyline,
      radiusMeters: 200,
    );
    state = state.copyWith(blitzerOnRoute: onRoute);
    debugPrint('[DrivingMode] ${onRoute.length} Blitzer auf der Route');
  }
